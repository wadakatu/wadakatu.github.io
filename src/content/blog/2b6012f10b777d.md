...
> **参考:** Contextual Bindingの基本について詳しく知りたい方は、[Laravel公式ドキュメント](https://laravel.com/docs/10.x/container#contextual-binding)をご覧ください。
...
**Lumen v10**: `when` メソッドを使ったContextual Bindingが、DIチェーン全体（間接的な依存関係）に適用される

**Lumen v11**: `when` メソッドで指定したクラスに直接DIされる場合のみ適用される
:::
...
DIチェーンを考慮して、実際にインターフェースを必要とする直接の依存先クラスを指定します。
:::

ここでのポイントは、`needs` メソッドの基準を `PaymentInterface` ではなく `MoneyService` に変更している点です。  
これにより、正しい `PaymentInterface`をDIした `MoneyService` が使用されるようになります。
